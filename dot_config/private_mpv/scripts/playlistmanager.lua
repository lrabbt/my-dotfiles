...
--END OF SAMPLE REPLACE ]=====]
	...
	--loadfiles at startup if there is 0 or 1 items in playlist, if 0 uses working dir for files
	...
	--resolve titles timeout
	resolve_titles_timeout = 5,
...
function save_playlist()
	...
	if not file then
		...
		while i < length do
			...
			local title = mp.get_property("playlist/" .. i .. "/title")
			if not title and url_table[filename] then
...
function resolve_titles()
	...
	while i < length do
		...
		then
			...
			local command = settings.youtube_dl
			local args = { command, "--no-playlist", "--flat-playlist", "-sJ", filename }
			...
			mp.add_timeout(settings.resolve_titles_timeout, function()
